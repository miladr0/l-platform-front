...
import {
  ...
  $setSearchKeys,
  REQUEST_INSERT_ANSWERS
  
} from "./customers.redux";
  ...
  render() {
    ...
    const columns = [
      ...
      },
      {
        ...
        render: (text, record) => (
          <span>
            <Link to={`/dashboard/customer/${record.id}`}>
              View Students
            </Link>
          </span>
        )
...
const mapStateToProps = state => {
  ...
  return {
    ...
    btnLoading: state.customers.btnLoading
  };
};
...
const mapDispatchToProps = dispatch => {
  return {
    ...
    $insertAnswers: () => {
      dispatch({ type: REQUEST_INSERT_ANSWERS });
    },
  };
};
